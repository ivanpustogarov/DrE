...
namespace {
  ...
  Environ("environ", cl::desc("Parse environ from given file (in \"env\" format)"));
	...
	cl::opt<std::string>
	ChipName("chip",cl::desc("MSP430 chip model (e.g. -chip=msp430g2553) "), cl::init(""));

	cl::opt<std::string>
	MemSpec("mmodel",cl::desc("MSP430 memory model folder (the one which contains "), cl::init(""));

	cl::opt<std::string>
	IntrSpec("imodel", cl::desc("Interrupt library"), cl::init(""));

	cl::opt<std::string>
	Plugin("plugpath", cl::desc("Interrupt library"), cl::init(""));
...
/* -- */
  cl::opt<unsigned>
  SummaryCollectTime("smmry-collect-time",
            cl::desc("Stop collectin path summary for a function after that number of seconds (default 20)"),
            cl::init(20));

  cl::opt<unsigned>
  SummariesPerRun("smmries-per-run",
            cl::desc("Stop collectin path summary for a function after this number of path summaries (per round) (defaul 3)"),
            cl::init(3));
...
void KleeHandler::processTestCase(const ExecutionState &state,
                                  const char *errorMessage,
                                  const char *errorSuffix) {
  //llvm::outs() << "DEBUG: KleeHandler::processTestCase(): entered function.\n";
  ...
  if (!NoOutput) {
    //llvm::outs() << "DEBUG: KleeHandler::processTestCase(): point1.\n";
    std::vector< std::pair<std::string, std::vector<unsigned char> > > out;
    bool success = m_interpreter->getSymbolicSolution(state, out);
    //llvm::outs() << "DEBUG: KleeHandler::processTestCase(): point2.\n";
    ...
    //llvm::outs() << "DEBUG: KleeHandler::processTestCase(): point3.\n";
    if (success) {
      //llvm::outs() << "DEBUG: KleeHandler::processTestCase(): point4 (success = true).\n";
      KTest b;
      b.numArgs = m_argc;
      b.args = m_argv;
      b.symArgvs = 0;
...
static Executor *theInterpreter = 0;
...
/** Construct Klee module from llvm module; optimize llvm module
 *
 * ... TODO: add parameters description
 * @param [out] kmodule Klee module that we contstruct based on llvm module
 * ...
 */
const Module *init_kmodule(llvm::Module *module,  const Interpreter::ModuleOptions &opts,
    InterpreterHandler *interpreterHandler, KModule **kmodule) 
{
 
  *kmodule = new KModule(module);
  assert(*kmodule && "new KModule() failed");
  ...
  //SpecialFunctionHandler *specialFunctionHandler = new SpecialFunctionHandler(*this);

  //specialFunctionHandler->prepare();
  /* The only reason we pass interpreterHandler here is that we need path to
   * the user's code */
...
int main(int argc, char **argv, char **envp) {
  ...
  Module *mainModule = 0;
#if LLVM_VERSION_CODE < LLVM_VERSION(3, 5)
  fprintf(stderr, "IVAN: Using llvm version bofore 3.5\n");
  ...
  llvm::outs() << "main(): creating STPSolver...\n";
  coreSolver = new STPSolver(UseForkedCoreSolver, CoreSolverOptimizeDivides);


  KleeHandler *handler = new KleeHandler(pArgc, pArgv);
  KModule *kmodule = NULL;
  /* This will create a new kmodule and write it to <kmodule> */
  const Module *finalModule = init_kmodule(mainModule,Opts,handler,&kmodule);
  assert(kmodule && "Error creating new kmodule");

  //interpreter_manager->setModule(mainModule);

  //KleeHandler *handler_d = new KleeHandler(pArgc, pArgv);
  //kmodule = new KModule(module);
  //kmodule->prepare(Opts, handler_d);

  //Interpreter::InterpreterOptions IOpts_d;
  //IOpts_d.MakeConcreteSymbolic = MakeConcreteSymbolic;
  //llvm::outs() << "main(): creating debug executor...\n";
  //Interpreter *interpreter_d = new Executor(IOpts_d, handler_d, coreSolver);
  //interpreter_d->setModule(mainModule, Opts);
  /*** DEBUG END ***/
  ...
  Executor *interpreter =
    theInterpreter = new Executor(IOpts, handler, coreSolver);
  ...
  llvm::outs() << "main(): setting new module for the main executor\n";
  //interpreter->setModule(kmodule);

  if (ChipName.empty())
    klee_error("Invalid target: no chip found for firmware\n");
  fprintf(stderr, "Using chip %s\n", ChipName.c_str());
  Architecture *arch = new Architecture(Plugin, MemSpec, IntrSpec, ChipName, mainModule);
  interpreter->setArch(arch);
...
/// IVAN ADDITIONS BEGIN: interpreter manager
  /* This constructor will run path_main_routine */
  PathManager *path_manager = new PathManager(StartFunction, TargetFunction, kmodule);
  llvm::outs() << "main(): Path Manager contrcuted; path_functions.size() = " << path_manager->path_functions.size() << "\n";
  llvm::outs() << "main(): creating InterpreterManager...\n";

  ForwardExecutor *forward_executor = new ForwardExecutor(IOpts, coreSolver, kmodule);
  forward_executor->setArch(arch);
  forward_executor->setPathManager(path_manager);
  forward_executor->target_function_name = TargetFunction;

  InterpreterManager *interpreter_manager = new InterpreterManager(IOpts, arch, kmodule, coreSolver);
  interpreter_manager->forward_interpreter = forward_executor;
  forward_executor->interpreter_manager = interpreter_manager;
  interpreter_manager->path_manager = path_manager;
  interpreter_manager->kmodule = kmodule;
  interpreter_manager->slave_search_freq = 0.33; // Choose state at random every 20th iteration

  interpreter_manager->collect_timeout = SummaryCollectTime; // Move to executing other function after 60 seconds
  interpreter_manager->smmries_per_run = SummariesPerRun; // Move to executing other function after 60 seconds

  interpreter_manager->startCompositSymbEx(mainFn, pArgc, pArgv, pEnvp);
...
/// IVAN ADDITIONS END
  ...
  if (!ReplayOutDir.empty() || !ReplayOutFile.empty()) 
  {
...
#endif
  } else { // This is what is going to be executed (Ivan)
    std::vector<KTest *> seeds;
    for (std::vector<std::string>::iterator
           it = SeedOutFile.begin(), ie = SeedOutFile.end();
         it != ie; ++it) {
      assert(0 && "SeedOutFile is not supported for now");
      KTest *out = kTest_fromFile(it->c_str());
      if (!out) {
        llvm::errs() << "KLEE: unable to open: " << *it << "\n";
        exit(1);
    ...
    for (std::vector<std::string>::iterator
         ...
         it != ie; ++it) {
      assert(0 && "SeedOutDir is not supported for now");
    ...
    if (!seeds.empty()) {
      assert(0 && "Seeds are not supported for now");
    ...
    fprintf(stderr, "IVAN: Executing interpreter->runFunctionAsMain\n");
    //interpreter->runFunctionAsMain(mainFn, pArgc, pArgv, pEnvp);
    fprintf(stderr, "IVAN: Executing interpreter->runFunctionAsNonMain\n");

    /*
    fprintf(stderr, "IVAN: Executing interpreter->startCompositSymbEx()\n");
    interpreter->path_main_routine(StartFunction, TargetFunction);
    interpreter->startCompositSymbEx(mainFn, pArgc, pArgv, pEnvp);*/
