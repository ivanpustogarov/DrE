...
bool TimingSolver::evaluate(const ExecutionState& state, ref<Expr> expr,
  ...
  //outs() << "IVAN: inside TimingSolver::evaluate()\n";
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(expr)) {
    outs() << "IVAN: TimingSolver::evaluate(): dealing with constant expression = " << CE->getAPValue() << "\n";
...
TimingSolver::getInitialValues(const ExecutionState& state, 
                               ...
                               std::vector< std::vector<unsigned char> >
                                 &result) {
  //llvm::outs() << "DEBUG: TimingSolver::getInitialValues(): inside.\n";
  if (objects.empty())
    return true;
