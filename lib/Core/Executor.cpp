...
/* ---- */
  cl::opt<bool>
  WithInterrupts("with-interrupts",
            cl::desc("Update global variables each basic block for backward symbex (as if they are changed inside ISR) (default=on)"),
            cl::init(true));

  cl::opt<bool>
  BreakOnFirstFit("smmry-break-on-firstfit",
            cl::desc("Stop collecting path summary for a function upon first fit (default true)"),
            cl::init(true));

  cl::opt<bool>
  ForwardOnly("symbex-forward-only",
           cl::desc("Don't collect summaries, run only forward execution (default is 0)."),
           cl::init(false));
...
Executor::Executor(const InterpreterOptions &opts, Solver *_coreSolver, KModule *_kmodule) :
  Interpreter(opts),
  kmodule(_kmodule),
  arch(0),
  interpreterHandler(0),
  searcher(0),
...
{
  this->solver = new TimingSolver(_coreSolver, true);
...
void Executor::executeAlloc(ExecutionState &state,
  ...
  llvm::outs() << "Executor::executeAlloc(): inside\n";
...
void Executor::setPathManager(PathManager *_path_manager)
{
  assert(!path_manager && "Changing path manager is not supported");
  //llvm::outs() << "executor->replayPath = " << replayPath << "\n";
