...
bool AddressSpace::resolveOne(const ref<ConstantExpr> &addr, 
  ...
  if (const MemoryMap::value_type *res = objects.lookup_previous(&hack)) {
    ...
    assert(mo->guest_address && "Memory object guest_address is not set for state");
...
bool AddressSpace::resolveOne(ExecutionState &state,
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(address)) {
    ...
    MemoryObject hack(example); /* We set both guest_address and host_address to example */
...
bool AddressSpace::resolve(ExecutionState &state,
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(p)) {
    llvm::outs() << "AddressSpace::resolve(): resolving constant expression\n";
    ObjectPair res;
...
void AddressSpace::copyOutConcretes() {
  for (MemoryMap::iterator it = objects.begin(), ie = objects.end(); 
    ...
    if (!mo->isUserSpecified) {
      ...
      assert(mo->host_address && "host_address cannot be NULL for using contrets");
...
/*  IVAN: function get_free_memchunk_at_guest() relies on tha AddressSpace.objects is sorted by guest_address
bool MemoryObjectLT::operator()(const MemoryObject *a, const MemoryObject *b) const {
  return a->address < b->address;
}*/

/*  IVAN: the following functions  rely on tha AddressSpace.objects is sorted by guest_address:
 *  get_free_memchunk_at_guest()
 *  resolveOne() */
bool MemoryObjectLT::operator()(const MemoryObject *a, const MemoryObject *b) const {
