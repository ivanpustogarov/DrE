...
public:
  unsigned id;
  /* Not used for msp430, we keep it to make Executor happy; TODO: make it equal to host_address*/
  uint64_t address; 
  // Non-zero, only if the memory object belongs to a state.
  uint64_t guest_address;
  // Points to the address returned by malloc() on the host
...
public:
  ...
  MemoryObject(uint64_t _guest_address, uint64_t _host_address, unsigned _size, 
  ...
  ref<ConstantExpr> getBaseExpr() const { 
    return ConstantExpr::create(guest_address, Context::get().getPointerWidth());
