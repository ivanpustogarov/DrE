...
extern "C" bool check_ADC10(klee::Architecture * arch, klee::ExecutionState * state, TimingSolver *solver)
{
  ...
  klee::ref<Expr> sr = arch->getSR(*state);
  {
    ...
    if(result)
    {
      ...
      llvm::outs() << "current ADC10CTL0 value = " << ADC10CTL0_val << "\n";
      ...
      if (false && result) /* and if SC bit (it's bit 0) is set */
      {
        llvm::outs() << "SC is set\n";
        ...
        llvm::outs() << "new ADC10CTL0 value = " << new_val << "\n";
        ret = true;
      } else
   ...
   * ADC10MEM register */
#if 0
  static unsigned int use_count_adcmem = 0;
  use_count_adcmem++;
  assert(arch->namedAddresses.count("ADC10MEM") || arch->namedAddresses.count("ADC10MEM0"));
  static ref<ConstantExpr> ADC10MEM_address = ConstantExpr::create(arch->namedAddresses["ADC10MEM"]->base, Context::get().getPointerWidth());;
  //if(arch->namedAddresses.count("ADC10MEM")) 
  //{
  //  ADC10MEM_address = ConstantExpr::create(arch->namedAddresses["ADC10MEM"]->base, Context::get().getPointerWidth());
  //  llvm::outs() << "ADC10MEM_address = " << ADC10MEM_address << "\n";
  //}
  //else
  //{
  //  ADC10MEM_address = ConstantExpr::create(arch->namedAddresses["ADC10MEM0"]->base, Context::get().getPointerWidth());
  //  llvm::outs() << "ADC10MEM_address = " << ADC10MEM_address << "\n";
  //}
  const Array * array = Array::CreateArray("ADC10MEM_arr" + llvm::utostr(use_count_adcmem), Expr::Int16);
  ref<Expr> new_adc10_value = Expr::createTempRead(array, Expr::Int16, true);
  llvm::outs() << "ADC10MEM_address = " << ADC10MEM_address << "\n";
  llvm::outs() << "writing ADC10MEM  = " << new_adc10_value << "\n";
