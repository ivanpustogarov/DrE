...
public:
  ...
  /// given object.
  static ref<Expr> createTempRead(const Array *array, Expr::Width w, 	bool peripheral=false);
...
public:
  ...
  static ref<ConstantExpr> create(uint64_t v, Width w) {
    ...
    //llvm::outs() << "v is " << v << "\n";
		//llvm::outs() << "w is " << w << "\n";
		//llvm::outs() << "trunc: " << bits64::truncateToNBits(v, w) << "\n";
...
public:
  UpdateList updates;
  ref<Expr> index;
  const bool fromPeripheral;

public:
  static ref<Expr> alloc(const UpdateList &updates, const ref<Expr> &index, bool periph=false) {
    ref<Expr> r(new ReadExpr(updates, index, periph));
    r->computeHash();
    return r;
  }
  
  static ref<Expr> create(const UpdateList &updates, ref<Expr> i, bool perip=false);
...
private:
  ReadExpr(const UpdateList &_updates, const ref<Expr> &_index, bool periph=false) : 
    updates(_updates), index(_index), fromPeripheral(periph) { assert(updates.root); }
